...
public class DefaultTeam {
	...
	// fonction utiliser pour reduire le nombre d'operation lors du calcul de la distance
...
//		return calculCercleMinAlgoNaif(inputPoints);
	...
	// l'implémentation d'algorithme naif ( c'est celle du prof avec quelques modification apporté ) 
...
//		System.out.println("calcul du cercle minimum terminer");
	...
	// implémentation de l'algorithme Welzl qui renvoie le circle couverant tout point de la liste, de rayon minimum
...
//		System.out.println("commencement du welzel");
	...
	public static Circle circleDuTriangle(Point p, Point q, Point s) {
		...
		double ux = (px2py2 * (q.y - s.y) 
				+ qx2qy2 * (s.y - p.y) 
				+ sx2sy2 * (p.y - q.y)) / D;
		double uy = (px2py2 * (s.x - q.x) 
				+ qx2qy2 * (p.x - s.x) 
				+ sx2sy2 * (q.x - p.x)) / D;
